...
namespace ExpressBase.ServiceStack.Services
{
    ...
    public class WebFormServices : EbBaseService
    {
        ...
        {
            string cols = string.Join(CharConstants.COMMA, listNamesAndTypes.Select(x => x.Name).ToArray());
        ...
        private string CreateDataReader(CreateWebFormTableRequest request, string cols)
        {
            ...
            drObj.Sql = "SELECT id,@colname@ FROM @tbl".Replace("@tbl", request.WebObj.TableName).Replace("@colname@", cols);
        ...
        private DVColumnCollection GetDVColumnCollection(List<TableColumnMeta> listNamesAndTypes, CreateWebFormTableRequest request)
        {
            ...
            foreach (TableColumnMeta column in listNamesAndTypes)
            {
                ...
                if (column.Type.EbDbType == EbDbTypes.String)
                    _col = new DVStringColumn { Data = index, Name = column.Name, sTitle = column.Name, Type = column.Type.EbDbType, bVisible = true, sWidth = "100px", ClassName = "tdheight" };
                    ...
                    _col = new DVNumericColumn { Data = index, Name = column.Name, sTitle = column.Name, Type = column.Type.EbDbType, bVisible = true, sWidth = "100px", ClassName = "tdheight dt-body-right" };
                else if (column.Type.EbDbType == EbDbTypes.Boolean)
                    _col = new DVBooleanColumn { Data = index, Name = column.Name, sTitle = column.Name, Type = column.Type.EbDbType, bVisible = true, sWidth = "100px", ClassName = "tdheight" };
                else if (column.Type.EbDbType == EbDbTypes.DateTime || column.Type.EbDbType == EbDbTypes.Date || column.Type.EbDbType == EbDbTypes.Time)
                    _col = new DVDateTimeColumn { Data = index, Name = column.Name, sTitle = column.Name, Type = column.Type.EbDbType, bVisible = true, sWidth = "100px", ClassName = "tdheight" };
