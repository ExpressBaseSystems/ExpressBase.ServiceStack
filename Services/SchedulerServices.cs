...
namespace ExpressBase.ServiceStack.Services
{
    public class SchedulerServices : EbBaseService
    {
        ...
        public GetAllUsersResponse Get(GetAllUsersRequest request)
        {
            ...
            {
                con.Open();
                string sql = @"SELECT id, firstname FROM eb_users WHERE statusid = 0 ;
...
";
                DbParameter[] parameters = { EbConnectionFactory.ObjectsDB.GetNewParameter("userids", EbDbTypes.String, request.UserIds),
               EbConnectionFactory.ObjectsDB.GetNewParameter("groupids", EbDbTypes.String,  request.UserGroupIds) };
