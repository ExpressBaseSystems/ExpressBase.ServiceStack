                ...
                if (!string.IsNullOrEmpty(col.LinkRefId) && (_isexcel == false))
                {
                    if (AllowLinkforZero)
                    {
                        if (col.LinkType == LinkTypeEnum.Popout)
                            ...
                            _formattedData = _formattedData = _formattedData + "&nbsp; <a  href= '#' oncontextmenu= 'return false' class ='tablelink' data-colindex='" + col.Data + "' data-link='" + col.LinkRefId + "' data-popup='true' data-data='" + _formattedData + "'></a>";
                    }
                }
