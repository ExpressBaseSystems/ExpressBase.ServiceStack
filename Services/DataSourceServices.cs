...
namespace ExpressBase.ServiceStack
{
    ...
    public class DataSourceService : EbBaseService
    {
        ...
        [CompressResponse]
        public DataSourceDataResponsebot Any(DataSourceDataRequestbot request)
        {
            this.Log.Info("data request");

            DataSourceDataResponsebot  dsresponse = null;
            ...
            Console.WriteLine("Before :  " + DateTime.Now);
            var dtStart = DateTime.Now;
            var _dataset = this.EbConnectionFactory.ObjectsDB.DoQueries2(_sql, parameters.ToArray<System.Data.Common.DbParameter>());
            ...
            if (_isPaged)
            {
                Int32.TryParse(_dataset.Tables[0].Rows[0].Row[0].ToString(), out _recordsTotal);
                Int32.TryParse(_dataset.Tables[0].Rows[0].Row[0].ToString(), out _recordsFiltered);
            }
            _recordsTotal = (_recordsTotal > 0) ? _recordsTotal : _dataset.Tables[0].Rows.Count;
            _recordsFiltered = (_recordsFiltered > 0) ? _recordsFiltered : _dataset.Tables[0].Rows.Count;
            //-- 

            dsresponse = new DataSourceDataResponsebot
            {
                Draw = request.Draw,
                //Data = (_dataset.Tables.Count > 1) ? _dataset.Tables[1].Rows : _dataset.Tables[0].Rows,
                RecordsTotal = _recordsTotal,
