    ...
    public class ChatbotServices : EbBaseService
    {
        ...
        public CreateBotFormTableResponse Any(CreateBotFormTableRequest request)// redirecting to Webform table create logic
        {
            ...
            if (string.IsNullOrEmpty(request.BotObj.WebFormRefId) && string.IsNullOrEmpty(_botForm?.WebFormRefId))// new object
            {
                ...
                CreateOrAlterBotFormColumns(WebForm);
            }
            else
            {
        ...
        private void SaveObjectRequest(EbObject obj, string Apps)
        {
            ...
            if (!string.IsNullOrEmpty(resp.Message))
                throw new FormException(resp.Message);
            if (obj is EbWebForm)
                CreateOrAlterBotFormColumns(obj as EbWebForm);
        }

        private void CreateOrAlterBotFormColumns(EbWebForm Form)
        {
            WebFormServices webservices = base.ResolveService<WebFormServices>();
            string msg = string.Empty;
            List<TableColumnMeta> list = new List<TableColumnMeta> { new TableColumnMeta { Name = "eb_created_aid", Type = this.EbConnectionFactory.DataDB.VendorDbTypes.Int32 } };
            webservices.CreateOrAlterTable(Form.TableName, list, ref msg);
