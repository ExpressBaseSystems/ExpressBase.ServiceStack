...
namespace ExpressBase.ServiceStack.MQServices
{
    ...
    {
        public SlackServiceInternal(IMessageProducer _mqp) : base(_mqp) { }
        ...
        //    }
        //}       
        public string Post(SlackCreateRequest request)
        {
            try
            {
                if (request.SolnId == CoreConstants.EXPRESSBASE)
                {
                    //Console.WriteLine("Inside EmailService/EmailServiceInternal in SS  Before Email To: " + request.To + "\nEmail Connections - infra: " + InfraConnectionFactory.EmailConnection.Capacity);
                    this.InfraConnectionFactory.ChatConnection.Send(request.To,  request.Message,  request.AttachmentReport, request.AttachmentName);
                }
                else
                {
                    ...
                    this.EbConnectionFactory.ChatConnection.Send(request.To, request.Message, request.AttachmentReport, request.AttachmentName);
                }
